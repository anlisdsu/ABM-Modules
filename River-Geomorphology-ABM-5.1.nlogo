...
;
; Major changes from the last version (Version 5.0)
; 1) Minor change in the flow-change function according to the rules of version 2 (see the Info tab)
; 2) A couple of changes on the plots: title, X and Y lables, data range of Y axis, size of plot, etc.
...
to flow-change
  ifelse (ticks = 27 or (ticks >= 59 and ticks <= 62)) [  ; Flow change reflects whether it rains & the river flow rises on some days
    set flow 70      ; Days 27 and 59, 60, 61, andn 62 are days with storms. The number 70 is just one between 65 and 310
    set storm? true
  ]
  [ set flow 40
    set storm? false
  ...
  [                         ; On days with a storm, the storm duration is set to different numbers according to Anne's data 
    if (ticks = 27 or ticks = 59) [  ; or ticks = 58 was added according to the rules of version 2 in Anne's email on 10/15/2014
      set storm-duration 1  ; used to be 3--change made on 10/15/2014
    ]    

    if (ticks >= 60 and ticks <= 62 ) [
      set storm-duration 48 ; Used to be 96--change made on 10/15/2014.
    ]
  ]
  ;show ticks
  ;show clock
...
@#$#@#$#@
Rules for model for sediment erosion (version 2):
1.	If flow <65 cfs (cubic feet per second), no net change in sediment thickness (surface elevation).

2.	If 65 cfs < flow < 311cfs:

For every TWO 15 min intervals during the day that 65 cfs < flow < 311 cfs, sediment thickness decreases by 0.113m from the most upstream segment.  [Downstream segments are not yet affected because the sediments that erode from the upstream segment will replace any downstream losses.]

On 9 Aug 2013, 65 cfs < flow < 311 cfs during three 15-min intervals, therefore sediment thickness decreases by 0.113 m.

On 11 Sept 2013, 65 cfs < flow < 311 cfs during two 15-min intervals, therefore sediment thickness decreases by 0.113 m.

On 12 Sept 2013, 65 cfs < flow < 311 cfs during 96 15-min intervals, therefore sediment thickness decreases by 0.113 m x 48 = 5.424 m.   [After sediment thickness decreases to 0 in first segment, decreasing sediment thickness continues in next segment downstream).

On 13 Sept 2013, 65 cfs < flow < 311 cfs during 96 15-min intervals, therefore sediment thickness decreases by 0.113 m x 48 = 5.424 m.   [After sediment thickness decreases to 0 in first segment, decreasing sediment thickness continues in next segment downstream).

On 14 Sept 2013, 65 cfs < flow < 311 cfs during 96 15-min intervals, therefore sediment thickness decreases by 0.113 m x 48 = 5.424 m.   [After sediment thickness decreases to 0 in first segment, decreasing sediment thickness continues in next segment downstream).

...
; 1) Versions 3.1 and 4.1 were combined--i.e., input data (x, y, ID, sediment-thick, hardrock-elev) are read  
;    from an external file rather than produced in the code, and at the same time:
; 2) All the procedures tested in V3.1 or earlier can be used based on the externally imported data.   
...
to setup
  ...
  if (file-exists? "StreamDataOutput.csv") [
    file-delete "StreamDataOutput.csv" ; A Version 3 feature: Each run (instead of each tick), delete the file and whatever inside it.
  ]
...
to flow-change
  ...
  [                         ; On days with a storm, the storm duration is set to different numbers according to Anne's data
    if (ticks = 27 ) [
      set storm-duration 3  ; Largely follow rules of version 1 (with miscalculation in days and Sept 11 storm duration)
    ]    
    ...
    if (ticks >= 60 and ticks <= 62 ) [
      set storm-duration 96 ; Number 96 was replaced with 5 to test the code.
    ]
  ]
  ;show ticks
  ;show clock

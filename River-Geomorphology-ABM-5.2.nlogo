...
;
; Major changes from the last version (Version 5.1). Still use rules of version 2 (see the Info tab) as v5.1
; 1) Added a function called "pave-channel"
; 2) Added the related global variables pave-time and build-pavement?, and put them in Interface as a slide bar and a switch
...
to flow-change
  ...
  [                         ; On days with a storm, the storm duration is set to different numbers according to Anne's data
    if (ticks = 27 or ticks = 59) [  ; or ticks = 58 was added according to Anne's email on 10/15/2014--still rules of Version 2

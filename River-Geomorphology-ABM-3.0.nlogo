...
;
; Major changes from the last version (v2.0)
; 1) A new function write-to-file was added to the code so that data can be written to an external csv file
; 2) Correspondingly, a command  file-delete "StreamDataOutput.csv" was added to setup.    
;    
...
patches-own [    ; No new comments. See comments in v1.0 and 2.0
 stream? 
 elevation
 hardrock-elev
 sediment-thick
...
segments-own [   ; No new comments. See comments in v1.0. Now not used
 my-ID
 elev
]
...
to setup
  ...
  setup-patches        ; This procedure is defined below.
  reset-ticks
  
  if (file-exists? "StreamDataOutput.csv") [
    file-delete "StreamDataOutput.csv" ; Version 3.0: Each run (instead of each tick), delete the file and whatever inside it.
  ]
...
to go                                     ; No new comments. See comments in v1.0 and 2.0
  if ticks >= 200 [ stop ]  
  set clock (clock + 1)
  flow-change
